...
class EQTransformerForPicker(nn.Module):
    """
...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        # Use zip for convs and pools, access paddings by index
        for i, (conv, pool) in enumerate(zip(self.convs, self.pools)):
            padding = self.paddings[i]  # Access padding by index
            
            x = torch.relu(conv(x))  # Apply convolution followed by ReLU
            if padding != 0:
                # Only pad right, use -1e10 as negative infinity
                x = F.pad(x, (0, padding), "constant", -1000000.0)  # Apply padding to the right
...
class Picker(EQTransformerForPicker):
    def __init__(self):
        super().__init__() 
...
# Load the pre-trained EqTransformer model from SeisBench (you can replace with your own model path)
model2 = EQTransformer.from_pretrained('stead').to(device)
...
"pickers/eqtransformer.stead.onnx", verbose=True, 
